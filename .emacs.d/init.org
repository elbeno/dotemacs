...
** Layout alists
#+begin_src emacs-lisp
(defun make-layout-alist (cols)
  (let* ((f (selected-frame))
         (pw (display-pixel-width))
         (cw (frame-char-width f))
         (l (- pw (* cols cw) 37)))
    `((top . 0) (left . ,l) (user-position . t)
      (width . ,cols) (height . ,default-frame-height))))
#+end_src
** For screencasting
#+begin_src emacs-lisp
...
(defun setup-normal-layout ()
  ...
  (modify-frame-parameters (selected-frame)
    (make-layout-alist column-wrap-hard)))
#+end_src
...
#+begin_src emacs-lisp
(defun setup-doublewide-layout-split ()
  (interactive)
  (modify-frame-parameters (selected-frame)
    (make-layout-alist (+ 3 (* 2 column-wrap-hard))))
...
(defun dock-frame-left ()
  (interactive)
  (modify-frame-parameters (selected-frame) '((top . 0) (left . 0) (user-position . t))))

(defun dock-frame-right ()
  (interactive)
  (let* ((f (selected-frame))
         (pw (frame-pixel-width f)))
    (modify-frame-parameters f
      `((top . 0 ) (left . ,(- (display-pixel-width) pw 16)) (user-position . t)))))
#+end_src
** Key bindings
#+begin_src emacs-lisp
...
(global-set-key "\C-c1" 'setup-normal-layout)
(global-set-key "\C-c2" 'setup-doublewide-layout-split)
...
(add-hook 'ediff-before-setup-hook 'start-ediff)
(add-hook 'ediff-quit-hook 'setup-normal-layout)
